...
;用法举例：sam_cluster_kmeans,5,10,0.01
PRO sam_cluster_kmeans, num_class, MaxIteration, minChangeThreshold
  ...
  for interation=0, MaxIteration-1 do begin
    ...
    for n=0, dims[2] do begin
     for j=0, dims[4] do begin
        t_sb = total(data[n,j,*]^2.0)
        for i=0, num_class-1 do begin
          ;计算每个像元与聚类中心的光谱余弦值sam
          t_sa = total(data[n,j,*]*cluster_center[*,i,interation])
          t_sc = total(cluster_center[*,i,interation]^2.0)
          dis_center[i] = t_sa/(sqrt(t_sb*t_sc))
        endfor
        ;sam值越大，距离越近
