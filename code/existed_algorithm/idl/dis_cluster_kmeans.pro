...
;用法为dis_cluster_kmeans,5,10,0.01
PRO dis_cluster_kmeans, num_class, MaxIteration, minChangeThreshold 

  ;选择需要聚类的图像
  infile = 'E:\method_project\qinghai\hyperion\EO1H1330342003247110PF_1T_processed_crop.tif'
  ...
  for interation=0, MaxIteration-1 do begin
    ...
    for n=0, dims[2] do begin
     for j=0, dims[4] do begin
          ;背景值不纳入分类的范围
          index_b = where(data[n,j,*] gt 0.0,count)
          ;判断如果所找到的序号的所有大于0的值的数目为176（波段数），则是有值的点
             for i=0, num_class-1 do begin
               ;计算每个像元与聚类中心的光谱距离余弦值sam_dis
               dis = total((data[n,j,*]-cluster_center[*,i,interation])^2.0)
               dis_center[i] = sqrt(dis)
             endfor
          ;dis值越小，距离越近
