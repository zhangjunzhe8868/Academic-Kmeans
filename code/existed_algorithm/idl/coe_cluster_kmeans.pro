...
PRO coe_cluster_kmeans, num_class, MaxIteration, minChangeThreshold 
  ...
  for interation=0, MaxIteration-1 do begin
    ...
    for n=0, dims[2] do begin
     for j=0, dims[4] do begin
        m_sb = mean(data[n,j,*])
        tsub_m_sb = total((data[n,j,*]-m_sb)^2.0)
        for i=0, num_class-1 do begin
          ;计算每个像元与聚类中心的相关系数coe
          m_sa = mean(cluster_center[*,i,interation])
          tsub_m_sa = total((cluster_center[*,i,interation]-m_sa)^2.0)
          tsub_m_ab = total((cluster_center[*,i,interation]-m_sa)*(data[n,j,*]-m_sb))
          dis_center[i] = tsub_m_ab/(sqrt(tsub_m_sa*tsub_m_sb))
        endfor
        ;coe的值越大，距离越近
        index = where(dis_center eq max(dis_center),count1)
